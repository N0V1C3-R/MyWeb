...
!function () {
    ...
    function a() {
        ...
        s.forEach(function (s) {
            for (s.x += s.xa,
                s.y += s.ya,
                s.xa *= s.x > f || s.x < 0 ? -1 : 1,
                s.ya *= s.y > e || s.y < 0 ? -1 : 1,
                n.fillRect(s.x - .5, s.y - .5, 1, 1),
                o = 0; o < u.length; o++)
                t = u[o],
                    s !== t && null !== t.x && null !== t.y && (h = s.x - t.x,
                        c = s.y - t.y,
                        l = h * h + c * c,
                        l < t.max && (t === i && l >= t.max / 2 && (s.x -= .03 * h,
                            s.y -= .03 * c),
                            v = (t.max - l) / t.max,
                            n.beginPath(),
                            n.lineWidth = v / 2,
                            n.strokeStyle = "rgba(" + r.c + "," + (v + .2) + ")",
                            n.moveTo(s.x, s.y),
                            n.lineTo(t.x, t.y),
                            n.stroke()));
    ...
    var t = document.createElement("canvas"), r = v(), y = "c_n" + r.l, n = t.getContext("2d"), f, e, p = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.oRequestAnimationFrame || window.msRequestAnimationFrame || function (n) {
    ...
    for (t.id = y,
        t.style.cssText = "position:fixed;top:0;left:0;z-index:" + r.z + ";opacity:" + r.o,
    ...
    if (window.innerWidth > 0) {
        (DeviceInfo.OS.Name == "Windows" || DeviceInfo.OS.Name == "Mac OS") && ($(".canvas").show(),
            setTimeout(function () {
                a()
            }, 100))
    }
